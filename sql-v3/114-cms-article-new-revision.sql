drop function if exists cms_article__new_revision;

create or replace function cms_article__new_revision (o jsonb) returns jsonb
as $$
declare o2 jsonb;
...
begin
  ...
  raise notice 'cms_article__new_revision o2:%',o2; -- NOTICE:  {"content_item__new":89212}
  raise notice 'cms_article__new_revision new_revision_id:%', new_revision_id; -- NOTICE:  {"content_item__new":89212}
  ...
  */
  ...
  insert into cms_articles (article_id)
  values (new_revision_id);

  o2 := o2 || jsonb_build_object('article_id', new_revision_id);
  return o2;
end;$$
language 'plpgsql';
